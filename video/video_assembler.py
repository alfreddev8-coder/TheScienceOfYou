...
def process_background_to_vertical(bg_path, target_duration):
    """
    ...
    bg = bg.resized((int(scale_w), int(scale_h)))
    ...
    if x2 > cw:
        x1, x2 = max(0, cw - TARGET_WIDTH), cw
    if y2 > ch:
        y1, y2 = max(0, ch - TARGET_HEIGHT), ch

    bg = bg.cropped(x1=x1, y1=y1, x2=x2, y2=y2)

    if bg.size != (TARGET_WIDTH, TARGET_HEIGHT):
        bg = bg.resized((TARGET_WIDTH, TARGET_HEIGHT))
...
def assemble_short(bg_video_path, audio_path, output_path):
    ...
    try:
        ...
        print("[Assembly] Rendering Short...")
...
def assemble_longform(clip_paths, audio_path, output_path):
    ...
    try:
        ...
        for c in clips:
            ...
            processed.append(c.resized((1920, 1080)))
...
def add_watermark_ffmpeg(video_path, text="TheScienceOfYou", opacity=0.20):
...
def apply_visual_enhancements(video_path):
...
def ensure_shorts_duration(video_path, max_dur=58.0):
